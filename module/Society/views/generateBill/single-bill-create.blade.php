...
@section('title', 'Generate Single Bill')
...
@section('content')
    ...
    <form action="{{ route('generateBill.single.store') }}" method="POST" class="form-horizontal"
        ...
        <div class="col-12 col-md-12 col-xs-6" style="overflow: hidden;">
            <div class="widget-box">
                <div class="widget-header">
                    <h4 class="widget-title">Generate Single Bill</h4>
                </div>
                ...
                <div class="row justify-content-center" style="margin-top: 15px; margin-left: 10px;">
                    ...
                    <div class="col-md-2 col-12" style="padding-left:15px; padding-right:40px;">
                        <div class="form-group">
                            ...
                            <select id="roadName" name="road" class="form-control select2" required>
                    ...
                    <div class="col-md-2 col-12" style="padding-left:15px; padding-right:40px;">
                        <div class="form-group">
                            <label for="plotName">Plot<span style="color: red;">*</span></label>
                            <select id="plotName" name="plot" class="form-control select2" required>
                                <option value="">Select Plot</option>
                            </select>
                        </div>
                    </div>

                    <!-- Flat Dropdown with search enabled -->
                    <div class="col-md-2 col-12" style="padding-left:15px; padding-right:40px;">
                        <div class="form-group">
                            <label for="flatName">Flat</label>
                            <select id="flatName" name="flat[]" class="form-control select2" multiple="multiple">
                        ...
                        <tbody>
                            ...
                            @foreach ($generateBills as $bill)
                                <tr>
                                    ...
                                    <td>{{ $bill->AssignPlot->plotName ?? '' }}</td>
                                    <td>
                                        {{ $bill->singleBillDetails->pluck('flat.flatName')->filter()->implode(', ') }}
                                    </td>
                                    <td>{{ $bill->total_monthlyDue ?? '' }}</td>
                                    <td>
                                        <a href="{{ route('single.generateBill.pdf', $bill->id) }}"
                                            style="display: inline-block; margin-right: 10px;" title="Receipt"
                                            target="_blank">
                                            <i class="fa fa-receipt"></i>
                                        </a>

                                        <a href="{{ route('single.generateBill.bill', ['id' => $bill->id, 'export_type' => 'export_pdf'] + request()->query()) }}"
                                            target="_blank" style="margin-right: 5px" title="Download" target="_blank">
                                            <i class="fa fa-download"></i>
                                        </a>
                                        <form id="delete-form-{{ $bill->id }}"
                                            action="{{ route('single.generateBill.delete', $bill->id) }}" title="Delete"
...
@section('js')
    ...
    <script>
        document.getElementById('generate-button').addEventListener('click', function() {
            ...
            fetch('{{ route('generateSingleBill.save') }}', {
                    method: 'POST',
                    headers: {
    ...
    <script>
        document.getElementById('not-generate-button').addEventListener('click', function() {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('generateSingleBill.truncate') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',

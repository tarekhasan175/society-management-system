...
@section('page-header')
    <i class="fa fa-list"></i> Fund Transfer Edit
@stop
@push('style')
    ...
    <div class="row">
        ...
        <!-- heading -->
            <div class="widget-box widget-color-white ui-sortable-handle clearfix" id="widget-box-7">
                ...
                <!-- INPUTS -->
                <form action="{{route('fund-transfers.update', $fundTransfer->id)}}" method="post">
                    @csrf @method('PUT')
                    ...
                    <div class="row" style="width: 100%; margin: 0 0 20px !important;">
                        ...
                        <!-- Date -->
                        @include('includes.inputs.date-field', ['name' => 'date', 'is_required' => 'required', 'value' => $fundTransfer->date])
                        ...
                        <!-- From Account -->
                                    ...
                                    <select id="from_account_id" name="from_account_id"
                                        ...
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}"
                                                ...
                                                {{ oldSelect('account_id', $account->id) }}
                                                {{ $account->id == $fundTransfer->from_account_id ? 'selected' : '' }}>
                                                {{ $account->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-sm-9">
                                    <select id="to_account_id" name="to_account_id"
                                        ...
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}"
                                                    data-balance="{{ $account->balance ?? 0 }}"
                                                {{ oldSelect('account_id', $fundTransfer->to_account_id) }}
                                                {{ $account->id == $fundTransfer->to_account_id ? 'selected' : '' }}>
                                                {{ $account->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <!-- Description -->
                        @include('includes.inputs.input-field', ['name' => 'description', 'is_required' => 1, 'value' => $fundTransfer->description])

                        <!-- Reference -->
                        @include('includes.inputs.input-field', ['name' => 'reference', 'value' => $fundTransfer->reference])

                        <!-- Amount -->
                        @include('includes.inputs.input-field', ['name' => 'amount', 'is_number' => true, 'is_required' => 1, 'value' => $fundTransfer->amount])
    ...
    <script>
        ...
        $(document).ready(function () {
            fromAccountId.change(function () {
                let balance = Number($(this).find('option:selected').data('balance'));

                if ($(this).val() == '{{$fundTransfer->from_account_id}}')
                    balance += {{$fundTransfer->amount}};

                $('.from-text-account-balance').text(Number(balance).toFixed(2))
            })
            ...
            toAccountId.change(function () {
                ...
                $('.to-text-account-balance').text(Number(balance).toFixed(2))
            })
            ...
            fromAccountId.trigger('change');
            toAccountId.trigger('change');
        });
